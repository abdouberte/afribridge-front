...
export const metadata: Metadata = {
  ...
  description:
    "Votre commande a bien été reçue. Suivez-la avec votre numéro de commande.",
};
...
export default async function ConfirmationPage({ searchParams }: Props) {
  const { ref, total, zone, name } = await searchParams;
      ...
      <div className="max-w-lg mx-auto px-4 pt-10 pb-16 flex flex-col items-center text-center gap-6">
        ...
        <div>
          ...
          <h1 className="text-2xl font-black text-afri-text leading-tight mb-2">
            {nameVal ? `Merci ${nameVal.split(" ")[0]} !` : "Commande reçue !"}
          </h1>
          <p className="text-sm font-semibold text-afri-text-3 leading-relaxed">
            Ton paiement a bien été reçu. On s'occupe d'acheter ton produit et
            de te l'envoyer sous 2 semaines.
          </p>
        </div>
        ...
        {(totalVal || zoneVal) && (
          <div
            className="w-full flex flex-col gap-0 border-2 border-afri-border rounded-2xl overflow-hidden bg-white"
          ...
          >
            {totalVal && (
              <div className="flex justify-between items-center px-4 py-3 border-b border-afri-border">
                ...
                <span className="text-base font-black text-afri-amber">
                  {totalVal} FCFA
                </span>
              </div>
            )}
            {zoneVal && (
              <div className="flex justify-between items-center px-4 py-3 border-b border-afri-border">
                ...
                <span className="text-sm font-bold text-afri-text">{zoneVal}</span>
              </div>
            )}
            <div className="flex justify-between items-center px-4 py-3">
              ...
              <span className="text-sm font-bold text-afri-text">
                ~2 semaines
              </span>
            </div>
          </div>
